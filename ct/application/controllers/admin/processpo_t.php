...
class Processpo_t extends MY_Controller  {
	//上傳圖檔檔名的前置字元
	var $prefix;
	var $data;
	var $pager_config;
...
SELECT 
	...
	d.qty as qty, a.tos, a.knum, a.back, a.tcs, a.cfs, a.qty as qty_paper, a.t,
	 a.tvendor, a.prt, a.prtvendor, a.sfc, a.sfcvendor, a.heat, a.heatvendor,
	  ...
	  a.toq, a.toq2, a.toq_n, a.tos2,a.cfqty2, a.ispodone, a.po_t_no, a.ispo_t_done
...
FROM `orderdetail` AS a 
    ...
    where d.isdel = '' and a.isorderdone = 'X' and a.isneedpo_t = 'X' and tvendor != ''
EOT;
...
SELECT 
	...
	d.qty as qty, a.tos, a.knum, a.back, a.tcs, a.cfs, a.qty as qty_paper, a.t,
	 a.tvendor, a.prt, a.prtvendor, a.sfc, a.sfcvendor, a.heat, a.heatvendor,
	  a.cf, a.cfvendor, a.pst, a.pstvendor, a.ga, a.gavendor, a.glu, a.gluvendor,
	  a.toq, a.toq2, a.toq_n, a.tos2,a.cfqty2, a.ispodone, a.po_t_no, a.ispo_t_done
          ,e.sname as vname, c.sname as cname, a.po_t_no, podate, a.orderno
FROM `orderdetail` AS a 
       JOIN product AS b ON a.fk_product = b.id 
       JOIN `order` as d on d.id = a.fk_order
	   join `customer` as c on a.fk_customer = c.id
	   join `vendor` as e on e.id = a.tvendor
	   JOIN po_t AS f ON a.po_t_no = f.id
	where d.isdel = '' and a.isorderdone = 'X' and a.isneedpo_t = 'X' and tvendor != ''  and  '{$today}' <= DATE_ADD(f.podate, INTERVAL 2 
WEEK) 
...
FROM  `orderdetail` AS a
JOIN  `vendor` AS b ON a.tvendor = b.id
...
EOT;
	...
	public function excel() {
		...
		$ids = implode(',', $ids);
		$sql = <<<EOT
		select a.orderno, b.name as pname, c.sname as cname, a.t, a.tos2,a.toq2, t.sname as tvendor
		from orderdetail as a  
		join product as b on a.fk_product = b.id
		join customer as c on a.fk_customer = c.id
		left join vendor as t on a.tvendor = t.id
		where a.po_t_no in ({$ids})
EOT;
		$res = $this->db->query($sql)->result_array();

		$html = $this->load->view('admin/processpo_t_blank_excel',array(
					'arrod'=> $res,
					'b' => $b,
					'e' => $e,
					'gentime' => getTodayStr(),
	...
	public function generate() {
		...
		$sql = <<<EOT
select a.id, a.remark, a.podate, b.t, b.tos2, b.toq2, 
	 b.tvendor, b.prt, b.prtvendor, b.sfc, b.sfcvendor, b.heat, b.heatvendor,
	  b.cf, b.cfvendor, b.pst, b.pstvendor, b.ga, b.gavendor, b.glu, b.gluvendor , b.orderno from po_t as a 
join orderdetail as b on  b.po_t_no = a.id
join vendor as c on c.id = b.tvendor
...
// echo $sql;
		...
		foreach($arrod_tmp as $k => $v){
			...
			//指送：面紙 > 印刷 > 表面 > 燙金 
			if($item['prtvendor'] != '') {
				$item['t_nextvendor'] = trim($item['prtvendor']);
			}
			if($item['t_nextvendor'] == '' && $item['sfcvendor'] != '') {
				$item['t_nextvendor'] = trim($item['sfcvendor']);
			}
			if($item['t_nextvendor'] == '' && $item['heatvendor'] != '') {
				$item['t_nextvendor'] = trim($item['heatvendor']);
			}
			/*
			if($item['t_nextvendor'] == '' && $item['cfvendor'] != '') {
				$item['t_nextvendor'] = $item['cfvendor'];
		...
		$html = $this->load->view('admin/processpo_t_blank',array(
					'arrod' => $notdone
				), true);
...
EOT;
			...
			FROM `orderdetail` AS a 
					...
					join `vendor` as e on e.id = a.tvendor
					JOIN po_t AS f ON a.po_t_no = f.id
				where d.isdel = '' and a.isorderdone = 'X' and a.isneedpo_t = 'X' and tvendor != ''  and 
