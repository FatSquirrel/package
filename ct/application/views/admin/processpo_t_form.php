...
採購日期:
    ...
    <table border="1" style="border-collapse:collapse;width:100%">
      ...
      <tbody ng-repeat="item in oditems | filter:Appc.filterSelected">
        <tr><td>{{item.sn}}</td><td>{{item.t}}</td><td>{{item.tos2}}</td><td>{{item.toq2}}</td><td>{{item.t_nextvendor_cn}}</td></tr>
      </tbody>
    </table>
   </div>
    <div class="div-edit-litsec">
      ...
      <div class="donegroup" ng-repeat="(key, group) in donerows" ng-mouseup="Appc.processChoose(key, group, $event);">
        ...
        <table>
          <tr ng-repeat="item in group">
            ...
            <td class="field3">{{item.t}}</td>
            <td class="field4">{{item.tos2}}</td>
            <td class="field5">{{item.toq2}}</td>
...
<script>
  ...
  controller('AppController',['$scope','$http',function($scope,$http){
    ...
    Appc.confirmEditing = function(){
      ...
      $http({
              method:'POST',
              url:'/ct/admin/processpo_t/ajax_processpo_save',
              data: $.param(oParam),
              headers: {'Content-Type': 'application/x-www-form-urlencoded'}
          }).
          then(function(res) {
              ...
              location.href = '/ct/admin/processpo_t/index';
          }, function(res) {
              //error
      });
    };
    ...
    Appc.filterLeft = function(item) {
      if(item.ispo_t_done == '' && item.tvendor == $scope.filteringVendor){
        return true;
      } else {  
        return false;
      }
